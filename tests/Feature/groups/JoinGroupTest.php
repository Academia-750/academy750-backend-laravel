...
class JoinGroupTest extends TestCase
{
    ...
    {
        $this->post("api/v1/group/99/join", ['user_id' => $this->student->id])->assertStatus(404);
    }

    /** @test */
    public function user_not_found_404(): void
    {
        $this->post("api/v1/group/{$this->group->id}/join", ['user_id' => 99])->assertStatus(404);
    }
    ...
    public function join_group_200(): void
    {
        ...
        $response = $this->post("api/v1/group/{$this->group->id}/join", ['user_id' => $student->id])->assertStatus(200)->json();
    ...
    public function can_join_two_groups_200(): void
    {
        ...
        $this->post("api/v1/group/{$group2->id}/join", ['user_id' => $student->id])->assertStatus(200);
    }
    ...
    public function cant_join_group_if_already_exists_409(): void
    {
        $student = User::factory()->student()->create();
        $this->post("api/v1/group/{$this->group->id}/join", ['user_id' => $student->id])->assertStatus(200);
        $this->post("api/v1/group/{$this->group->id}/join", ['user_id' => $student->id])->assertStatus(409);
    }
    ...
    {
        $student = User::factory()->student()->create();
        $response = $this->post("api/v1/group/{$this->group->id}/join", ['user_id' => $student->id])->json();
        // Discharge the user
        ...
        // Re join the group
        $response2 = $this->post("api/v1/group/{$this->group->id}/join", ['user_id' => $student->id])->json();
