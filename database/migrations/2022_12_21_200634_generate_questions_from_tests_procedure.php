...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        BEGIN
                ...
                (SELECT testable_id FROM testables s, users u, tests t WHERE t.id=s.test_id AND u.id=t.user_id AND t.user_id=id_usuario AND t.id=id_test)
                AND p.id NOT IN(SELECT m.question_id from question_test m, users u, tests t WHERE u.id=t.user_id AND t.id=m.test_id AND t.test_type=tipo_cuestionario AND u.id=id_usuario)
                ...
                SELECT p.id from questions p WHERE p.questionable_id IN
                (SELECT testable_id FROM testables s, users u, tests t WHERE t.id=s.test_id AND u.id=t.user_id AND t.user_id=id_usuario AND t.id=id_test)
                AND p.its_for_card_memory='yes' ORDER BY RAND() LIMIT numero_preguntas_solicitadas ;
        ...
        BEGIN
                ...
                SELECT p.id from questions p WHERE p.questionable_id IN
                (SELECT testable_id FROM testables s, users u, tests t WHERE t.id=s.test_id AND u.id=t.user_id AND t.user_id=id_usuario AND t.id=id_test)
                AND p.its_for_test='yes' ORDER BY RAND() LIMIT numero_preguntas_solicitadas ;
