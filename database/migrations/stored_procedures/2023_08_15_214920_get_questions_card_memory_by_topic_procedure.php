...
return new class extends Migration {
    ...
    public function up()
    {
        ...
        $procedure = "DROP PROCEDURE IF EXISTS `get_questions_card_memory_by_topic_procedure`";
        DB::unprepared($procedure);

        $procedure = "CREATE PROCEDURE `get_questions_card_memory_by_topic_procedure`(
...
BEGIN
            ...
            /*INSERTAMOS EN TMP_TOCPIS EL NUMERO DE PREGUNTAS QUE TIENE CADA TEMA DE LOS QUE NOS HAN LLEGADO POR PARAMETRO DE ENTRADA.
            SE HACE UN UNION PARA JUNTAR LAS PREGUNTAS QUE SON DEL TEMA Y LAS PREGUNTAS QUE SEAN DE LOS SUBTEMAS*/
            ...
            FROM
              (
                SELECT DISTINCT q.questionable_id as topic_id,
            ...
            /*SI EL NUMERO DE TEMAS ES MAYOR QUE EL NUMERO DE PREGUNTAS, ELEGIMOS UN TEMA AL AZAR PARA BUSCAR LAS PREGUNTAS*/
            IF ( SELECT COUNT(*) FROM tmp_topics) > num_preguntas THEN INSERT INTO tmp_topics_selected
            	SELECT topic_uuid,
              		   total_questions
            	FROM   tmp_topics
            	ORDER BY RAND()
            ...
            /*DIVIDMOS EL NUMERO DE PREGUNTAS QUE SE SOLICITAN ENTRE EL NUMERO DE TEMAS*/
            SET npregs_by_topic = (SELECT count(*) FROM tmp_topics);
            SET npregs_by_topic := CASE WHEN npregs_by_topic != 0 THEN num_preguntas / npregs_by_topic ELSE 0 END;
            ...
            read_loop : LOOP FETCH cur1 INTO v_id;
            	...
            	DROP TEMPORARY TABLE IF EXISTS tmp_table_showed;
	            ...
	            DROP TEMPORARY TABLE IF EXISTS tmp_table_not_showed;
	            ...
	            INSERT INTO tmp_table_not_showed
	            	SELECT distinct q.id
	            	FROM questions q
	              	INNER JOIN subtopics st ON st.id = q.questionable_id
	              	INNER JOIN oppositionables op ON op.oppositionable_id = st.id
            	...
            	SET c := (SELECT count(*) FROM tmp_table_not_showed);

            	INSERT INTO tmp_selected_questions
            		SELECT id
            		FROM tmp_table_not_showed
            		ORDER BY RAND()
            		LIMIT npregs_by_topic;
            	...
            	DROP TEMPORARY TABLE tmp_table_showed;
            	DROP TEMPORARY TABLE tmp_table_not_showed;
