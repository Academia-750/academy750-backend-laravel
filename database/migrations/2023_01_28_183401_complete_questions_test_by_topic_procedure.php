...
return new class extends Migration
{
    ...
    public function up()
    {
        $procedure1= "DROP PROCEDURE IF EXISTS `complete_questions_test_by_topic`;
        CREATE PROCEDURE `complete_questions_test_by_topic`(
        ...
        WHERE m.question_id=q.id AND
            ...
            t.user_id=id_usuario AND t.id=m.test_id AND t.test_type='test' AND q.its_for_test='yes';
      ...
      SELECT p.id from questions p
           ...
           (select t.id from topics t where t.id=id_tema union select s.id from subtopics s, oppositionables o where o.opposition_id=id_oposicion and o.oppositionable_id=s.id and s.topic_id=id_tema)
            AND p.id not in
            (SELECT question_id FROM question_test q, tests t where t.user_id=id_usuario and t.id=q.test_id and t.test_type='test' and q.have_been_show_test='yes') AND NOT FIND_IN_SET(p.id,excluidos)
